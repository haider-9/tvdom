<script lang="ts">
  ...
  async function loadPosts(refresh = false) {
    try {
      ...
      const response = await fetch(
        `/api/posts?page=${refresh ? 1 : currentPage}&limit=20`,
      );
      ...
      if (refresh || currentPage === 1) {
        communityStats.totalPosts = data.pagination?.total || posts.length;
        const uniqueUsers = new Set(posts.map((p) => p.userId));
        communityStats.totalUsers = uniqueUsers.size;

        const today = new Date();
        today.setHours(0, 0, 0, 0);
        communityStats.activeToday = posts.filter((p) => {
  ...
  function filterPosts() {
    if (activeFilter === "latest") {
      return posts.map(parsePostData);
  ...
  function parsePostData(post: any) {
    ...
    if (typeof post.images === "string") {
      try {
        parsedPost.images = JSON.parse(post.images);
      } catch (e) {
        parsedPost.images = [];
      }
    }
    ...
    if (typeof post.likes === "string") {
      try {
        parsedPost.likes = JSON.parse(post.likes);
      } catch (e) {
        parsedPost.likes = [];
      }
    }
  ...
  function getLikesArray(post: any) {
    if (typeof post.likes === "string") {
      try {
  ...
  async function toggleLike(postId: string) {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        const post = posts.find((p) => (p._id?.toString() || p.$id) === postId);
        if (post) {
          const currentLikes =
            typeof post.likes === "string"
              ...
              : post.likes || [];
          if (data.liked) {
            post.likes = JSON.stringify([...currentLikes, userId]);
          } else {
            post.likes = JSON.stringify(
  ...
  async function deletePost(postId: string) {
    try {
      const userId = userStore.user?._id || userStore.user?.id;
      const response = await fetch(
        `/api/posts?postId=${postId}&userId=${userId}`,
        {
          method: "DELETE",
        },
      );
      ...
      if (response.ok) {
        posts = posts.filter((p) => (p._id?.toString() || p.$id) !== postId);
  ...
  // Helper function to get comment text for a post
  function getCommentText(postId: string): string {
    return commentTexts[postId] || "";
  }

  // Helper function to set comment text for a post
  function setCommentText(postId: string, value: string) {
    commentTexts[postId] = value;
    commentTexts = { ...commentTexts }; // Trigger reactivity
  }
  ...
  function isLiked(post: any) {
    const userId = userStore.user?._id || userStore.user?.id;
    const likes = getLikesArray(post);
...
<div class="community-page">
  <div class="container">
          ...
          <div class="create-post-header">
            ...
            <div class="create-post-input-wrapper">
              ...
              {#if newPostContent.length > 0}
                <div
                  class="char-count"
                  class:warning={newPostContent.length > 450}
                >
    ...
    {:else}
      <Card.Root>
        <Card.Content class="signin-prompt py-6">
          ...
          <Button onclick={() => goto("/login")} size="lg" class="mx-auto"
            >Sign In</Button
          >
        </Card.Content>
      </Card.Root>
    ...
    <div class="posts-feed">
      ...
      {:else}
        {#each filterPosts() as post, index (post._id || post.$id)}
          ...
          <Card.Root>
            <Card.Content class="p-0">
              <div class="post-header">
                  ...
                  <div class="user-info">
                    <div class="user-name-row">
                      <span class="user-name"
                        >{post.user?.displayName || "Unknown User"}</span
                      >
                    ...
                    <div class="post-meta">
                      <span class="username"
                        >@{post.user?.username || "unknown"}</span
                      >
                      <span class="separator">•</span>
                      <span class="post-time"
                        >{formatDate(post.$createdAt || post.createdAt)}</span
                      >
                    </div>
                  </div>
                </a>
                {#if isOwnPost(post)}
                  <Button
                    variant="ghost"
                    size="sm"
                    onclick={() => openDeleteDialog(postId)}
                    class="delete-btn"
                  >
              ...
              <div class="post-body">
                ...
                {#if getLikesArray(post).length > 0 || post.commentCount > 0}
                  <div class="engagement-stats">
                    {#if getLikesArray(post).length > 0}
                      <span class="engagement-item">
                        <Heart class="w-3.5 h-3.5 fill-current text-red-500" />
                        {getLikesArray(post).length}
                        {getLikesArray(post).length === 1 ? "like" : "likes"}
                      </span>
                    {/if}
              ...
              <div class="post-actions-bar">
                ...
                >
                  <Heart
                    class="w-5 h-5 {isLiked(post) ? 'fill-current' : ''}"
                  />
                  <span>{getLikesArray(post).length}</span>
                </button>
                <button
                  ...
                  {#if userStore.isAuthenticated}
                    <div class="add-comment">
                      <div class="add-comment-wrapper">
                        ...
                        {/if}
                        <Textarea
                          value={getCommentText(postId)}
                          oninput={(e) =>
                            setCommentText(postId, e.currentTarget.value)}
                      ...
                      <div class="add-comment-actions">
                        <Button
                          ...
                          disabled={!getCommentText(postId)?.trim()}
                        >
                          <Send class="w-3 h-3" />
                          Comment
                        </Button>
      ...
      {#if isLoadingMore}
        <div class="loading-more col-span-full">
